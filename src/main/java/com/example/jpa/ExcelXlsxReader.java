...
public class ExcelXlsxReader extends DefaultHandler {
    ...
    //处理总后得结果集
    private Map<String,Integer> totalResult = new HashMap<>();
     ...
     * @throws Exception
     */
    public Entity process(String filename) throws Exception {
        filePath = filename;
...
//        return totalRows; //返回该excel文件的总行数，不包括首列和空行
        ...
        entity.setExcelString(totalSet);
        entity.setStringIntegerMap(totalResult);
        return entity;
    }
        ...
        if (isTElement) {//这个程序没经过
            ...
            if ("row".equals(name)) {
                ...
                if (flag&&curRow!=1){ //该行不为空行且该行不是第一行，则发送（第一行为列名，不需要）
                    Entity result =
                            ExcelReaderUtil.sendRows(filePath,
                            sheetName,
                            sheetIndex
                            , curRow, cellList);
                    totalSet.addAll(result.getExcelString());
                    //获取到列表
                    for(String list:result.getCity()){
                        Integer integer = totalResult.get(list);
                        if(integer == null){
                            totalResult.put(list,1);
                        }else {
                            Integer num = totalResult.get(list) + 1;
                            totalResult.put(list,num);
                        }
                    }
                    totalRows++;
                }
